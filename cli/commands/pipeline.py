...
Commands:
    ...
    ude pipeline new               — scaffold YAML + dbt model stubs interactively
...
app = typer.Typer(help="Pipeline management — list, inspect, new, enable, disable")
...
def inspect(
    ...
    if batch:
        ...
        batch_rows = [
            ...
            ("dbt run status", "[success]pass[/success]" if batch.get("dbt_passed") else "[error]fail[/error]"),
...
def new(
    ...
    pipeline_id: Optional[str] = typer.Option(
        None, "--id",
        help="Pipeline ID (e.g. customers). Prompted if not provided."
    ),
) -> None:
    """
    Scaffold a new pipeline interactively.

    Creates:
      - config/pipelines/{id}.yml
      - dbt/models/staging/{id}_staged.sql
      - dbt/models/marts/dim_{id}.sql          (SCD Type 1)
    ...
    console.print()
    print_info("Gathering field definitions. Enter fields one by one.")
    print_info("Type [bold]done[/bold] when finished.\n")
    ...
    if not fields:
        print_warning("No fields defined. You can add them manually to the YAML later.")

    # Always add updated_at if not present
    ...
    print_success(f"Pipeline '{pid}' scaffolded.")
    console.print()
    ...
    console.print()
    console.print("  Next: add your source to [bold]dbt/models/staging/_sources.yml[/bold]")
...
def enable(
    ...
    from cli.client.pipeline import PipelineClient
    client = PipelineClient(ude_ctx.config)
    client.set_enabled(pipeline_id, enabled=True)
...
def disable(
    ...
    client.set_enabled(pipeline_id, enabled=False)
    print_warning(f"Pipeline '{pipeline_id}' paused.")
    print_info("Resume with: ude pipeline enable " + pipeline_id)